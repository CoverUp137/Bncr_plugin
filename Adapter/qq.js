...
const jsonSchema = BncrCreateSchema.object({
  ...
  autoAcceptFriend: BncrCreateSchema.boolean().setTitle('自动接受好友请求').setDescription('将会自动接受好友请求').setDefault(false),
  autoAcceptGroupJoin: BncrCreateSchema.boolean().setTitle('自动同意加群申请').setDescription('将会自动同意他人加群申请，需要机器人为群管理员').setDefault(false),
  welcomeMessage: BncrCreateSchema.string().setTitle('入群欢迎词').setDescription('设置自定义入群欢迎词').setDefault('欢迎加入本群！ 你好'),
});
...
async function ws(qq) {
  ...
  router.ws('/api/bot/qqws', ws => {
    ws.on('message', msg => {
      ...
      if (body.echo) {
        for (const e of listArr) {
          ...
          if (body.status && body.status === 'ok') return e.eventS.emit(e.uuid, body.data.message_id.toString());
        ...
        ws.send(
          JSON.stringify({
            ...
            params: {
              ...
              message: ConfigDB.userConfig.welcomeMessage,
            },
          })
        );
      }
    ...
    qq.reply = async function (replyInfo) {
      try {
        ...
        +replyInfo.groupId ? (body.params.group_id = replyInfo.groupId) : (body.params.user_id = replyInfo.userId);
  ...
  /**向/api/系统路由中添加路由 */
  router.get('/api/bot/qqws', (req, res) => res.send({ msg: '这是Bncr 外置qq Api接口，你的get请求测试正常~，请用ws交互数据' }));
  router.post('/api/bot/qqws', async (req, res) => res.send({ msg: '这是Bncr 外置qq Api接口，你的post请求测试正常~，请用ws交互数据' }));
...
async function http(qq) {
  ...
  router.post('/api/bot/qqHttp', async (req, res) => {
    ...
    if (body.post_type === 'request' && body.request_type === 'group' && (body.sub_type === 'add' || body.sub_type === 'invite') && ConfigDB.userConfig.autoAcceptGroupJoin) {
      requestPost('/set_group_add_request', {
        flag: body.flag,
        sub_type: body.sub_type,
        approve: true,
      });
    ...
    if (body.post_type === 'request' && body.request_type === 'friend' && ConfigDB.userConfig.autoAcceptFriend) {
      requestPost('/set_friend_add_request', {
        flag: body.flag,
        approve: true,
      });
    }
